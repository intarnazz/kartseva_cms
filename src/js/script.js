...
import * as change from "../modules/changePage.js";
{
  ...
  const getFromPage = () => {
    return {
      ...
      sum: document.querySelectorAll('.cms-main__container__table-form__table__row__col-sum'),
  ...
  const init = () => {
    ...
    allConst.allForm.addEventListener("submit", (e) => {
      ...
      obj.push({
        ...
        get value() {
          if (discount) {
            return (this.count * this.price) - ((this.count * this.price) * (discount / 100));
          } else {
    ...
    allConst.inputs.forEach(elem => {
      elem.addEventListener('change', () => {
        ...
        const count = formData.get("count");
        const price = formData.get("price");
        document.getElementById('modalSum').textContent = '$ ' + change.calculateSum(count, price, discount)
      })
    });
